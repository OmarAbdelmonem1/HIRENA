...
export default function CompanyJobDetails() {
  ...
  useEffect(() => {
    Promise.all([getCompanyJob(id), getCompanyJobAnalytics(id)])
      .then(([jobData, analyticsData]) => {
        setJob(jobData);
  ...
  if (!job) return <p className="table-message">Loading job…</p>;
  return (
    <div className="admin-page">
      <header className="admin-header">
        ...
        <div className="modal-actions">
          <button onClick={() => navigate("/company/jobs")}>← Back</button>
          <button
      ...
      <section className="metric-grid job-stats">
        <article className="metric-card">
          <div className="metric-copy plain">
            ...
            <strong>
              {analytics?.uniqueViewers ?? job.uniqueViewers ?? 0}
            </strong>
          </div>
        </article>
        <article className="metric-card">
        ...
        <article className="metric-card">
          <div className="metric-copy plain">
            ...
            <strong className="tone-green">
              {analytics?.admittedApplications ?? 0}
            </strong>
          </div>
        </article>
        <article className="metric-card">
      ...
      <section className="panel detail-panel">
        <div className="detail-meta">
          <span className="status-badge">{text(job.status)}</span>
          <span>{text(job.employmentType)}</span>
          <span>{job.location || "Location not specified"}</span>
          <span>{job.experienceRequired ?? 0} years experience</span>
        </div>
        <h2>Description</h2>
        ...
        <button
          className="primary-button"
          onClick={() => navigate(`/company/applications?jobId=${id}`)}
        >
          View applications
        </button>
      </section>
